...
def test_read_note_incorrect_id_2(test_app, monkeypatch):
    async def mock_get(id):
        return None
...
def test_create_note_invalid_json_2(test_app):
    response = test_app.post("/notes/", content=json.dumps({"title": "something"}))
    assert response.status_code == 422
...
def test_remove_note_incorrect_id_2(test_app, monkeypatch):
    async def mock_get(id):
        return None
